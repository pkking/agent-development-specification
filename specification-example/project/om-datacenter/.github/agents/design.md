...
- `/tmp/opencode/requirement_analysis.md` —— Workflow A 写、已评审合入的「需求分析说明书」（任务拆解 / 可量化验收标准 / need\_\* 标签结论）。**有就以它为准判路由 + 定验收标准**；不存在则以 issue 内容为准
...
1. **判路由**，写 `/tmp/opencode/route.json`（严格一行 JSON）：
   ...
   ```json
   {
     "mode": "normal|add-community|none",
