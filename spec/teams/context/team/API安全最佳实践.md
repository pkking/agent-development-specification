...
app.get('/api/auth/google', (req, res) => {
  ...
  authUrl.searchParams.append('redirect_uri', `${process.env.APP_URL}/api/auth/google/callback`);
...
app.get('/api/posts/:id', (req, res) => {
  const post = getPost(req.params.id);
  res.send(`<h1>${escapeHtml(post.title)}</h1><p>${escapeHtml(post.content)}</p>`);
});
...
app.post('/api/users', [
  ...
  body('password')
    ...
    .matches(/[A-Z]/)
    .matches(/[0-9]/)
    .withMessage('Password must be at least 8 chars with uppercase and number'),
  body('age')
    .isInt({ min: 0, max: 150 })
...
// 使用
const encryptionKey = crypto.scryptSync(process.env.ENCRYPTION_PASSWORD, 'salt', 32);
